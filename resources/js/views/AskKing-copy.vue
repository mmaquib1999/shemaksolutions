      ...
      <div style="flex:1;">
        ...
        <div class="dashboard-stats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px;">
          ...
          <div class="card" style="padding:16px;">
            ...
            <div style="font-size:24px;font-weight:bold;">Claude</div>
            <div style="font-size:11px;color:#64748b;">Anthropic</div>
          </div>
          ...
          <div style="padding:20px;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:16px;flex-wrap:wrap;gap:12px;">
              <div style="display:flex;gap:6px;flex-wrap:wrap;">
                <button v-for="t in quickTags" :key="t" @click="handleQuery(t + ' safety requirements')" style="padding:6px 12px;border-radius:6px;border:1px solid rgba(71,85,105,0.3);background:transparent;color:#94a3b8;font-size:12px;cursor:pointer;">
                  {{ t }}
                </button>
              </div>
              <div style="display:flex;gap:8px;">
                <button @click="submitQuery" class="btn" style="padding:8px 14px;border-radius:8px;background:linear-gradient(135deg,#0ea5e9,#06b6d4);border:none;color:white;cursor:pointer;">🚀 Ask K.I.N.G.</button>
              </div>
            </div>
          </div>
    ...
    <div style="margin-top:20px;">
      <div class="card" style="padding:12px;">
        ...
        <div style="max-height:320px;overflow-y:auto;">
          ...
          <div v-else>
            <div v-for="(q, idx) in recentHistory" :key="q.id" style="padding:12px;border-bottom:1px solid rgba(71,85,105,0.3);">
              <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                <span style="font-weight:600;font-size:13px;">{{ q.query.length > 60 ? q.query.substring(0, 60) + '...' : q.query }}</span>
                <span style="font-size:11px;color:#64748b;">{{ formatDate(q.timestamp) }}</span>
              </div>
              <div style="display:flex;gap:8px;align-items:center;">
                <span class="badge" style="background:rgba(99,102,241,0.2);color:#818cf8;font-size:10px;padding:4px 8px;border-radius:6px;">{{ q.industry }}</span>
                <span style="font-size:11px;color:#64748b;">by {{ q.user }}</span>
                <div style="margin-left:auto;display:flex;gap:8px;">
                  <button @click="replayQuery(q)" style="background:none;border:1px solid rgba(71,85,105,0.12);padding:6px 8px;border-radius:6px;cursor:pointer;">↺ Replay</button>
                  <button @click="downloadQueryText(q)" style="background:none;border:1px solid rgba(71,85,105,0.12);padding:6px 8px;border-radius:6px;cursor:pointer;">⬇</button>
                </div>
              </div>
            </div>
          </div>
...
/**
 * AskKing.vue
 * Single-page conversion of the "Ask K.I.N.G." tab.
 * - Hardcoded industries & prompts (Option A)
 * - Hardcoded demo user
 * - Local state for query history
 * - Exports as text blobs (mirrors original behavior)
 */
...
const teamMembers = ref([
  { id: 1, name: 'Claudino Nelson', email: 'claudino@shemaksolutions.ca', role: 'owner', queries: 1847, badges: ['loto', 'ppe', 'fire'], streak: 12 },
  { id: 2, name: 'Sarah Johnson', email: 'sarah@company.com', role: 'admin', queries: 623, badges: ['electrical'], streak: 8 }
])
...
/* compute current industry object */
const currentIndustry = computed(() => industries.find(i => i.id === selectedIndustry.value) || null)
...
function showToast(msg) {
  ...
  t.style.cssText = 'position:fixed;bottom:24px;right:24px;background:linear-gradient(135deg,#10b981,#059669);color:#fff;padding:12px 20px;border-radius:10px;font-size:14px;font-weight:500;z-index:1000;animation:slideUp 0.3s ease-out;box-shadow:0 8px 24px rgba(16,185,129,0.4);'
  t.textContent = msg
...
function isHighRiskQuery(q) {
  return /loto|lockout|tagout|confined\s*space|electrical|high\s*voltage|arc\s*flash|energized/i.test(q)
}

/* Main handler */
function handleQuery(query) {
  // set input value (like original)
  ...
  if (ra) {
    ra.innerHTML = `<div style="padding:20px;background:rgba(15,23,42,0.5);border-top:1px solid rgba(71,85,105,0.3);"><div style="display:flex;align-items:center;gap:8px;color:#22d3ee;"><span style="display:inline-block;animation:spin 1s linear infinite;">⟳</span> Processing with K.I.N.G. Framework v17.0...</div></div>`
  }
    ...
    <div style="padding:20px;background:rgba(15,23,42,0.5);border-top:1px solid rgba(71,85,105,0.3);">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:12px;">
        <div style="display:flex;align-items:center;gap:8px;color:#10b981;">
          ...
          <span style="font-size:11px;color:#64748b;margin-left:8px;">${mockUsage.avg_response_time}s</span>
          ${highRisk ? `<span class="badge" style="background:rgba(239,68,68,0.2);color:#ef4444;margin-left:8px;padding:4px 8px;border-radius:8px;">⚠️ HIGH RISK</span>` : ''}
        </div>
        <div style="display:flex;gap:8px;">
          ...
          <button id="export-jsa-btn" style="${highRisk ? 'padding:10px 16px;font-size:13px;background:linear-gradient(135deg,#ef4444,#dc2626);border:none;color:white;border-radius:8px;animation:pulse-glow 2s ease-in-out infinite;cursor:pointer;' : 'padding:8px 12px;border-radius:8px;border:1px solid rgba(71,85,105,0.12);background:transparent;cursor:pointer;'}">📋 Create JSA${highRisk ? ' ⚠️' : ''}</button>
        </div>
      </div>
      <pre style="white-space:pre-wrap;font-family:ui-monospace,monospace;font-size:13px;line-height:1.6;padding:16px;background:rgba(15,23,42,0.8);border:1px solid ${highRisk ? 'rgba(239,68,68,0.3)' : 'rgba(71,85,105,0.3)'};border-radius:10px;color:#e2e8f0;margin:0;overflow-x:auto;">${escapeHtml(responseText)}</pre>
      ${highRisk ? `<div style="margin-top:12px;padding:12px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:8px;display:flex;align-items:center;gap:10px;"><span style="font-size:20px;">⚠️</span><div><div style="font-weight:600;color:#f87171;">High-Risk Activity Detected</div><div style="font-size:12px;color:#94a3b8;">A Job Safety Analysis (JSA) is strongly recommended before proceeding.</div></div></div>` : ''}
    </div>
  `
...
/* Export functions (mirror original behavior: produce text blobs) */
function exportToPDF() {
  ...
  a.href = url
  a.download = `KING-Report-${new Date().toISOString().split('T')[0]}.txt` // original used .txt
  a.click()
...
/* simple helper to export all audit log (like original's summary/full) */
...
function getROICard() {
  ...
  const card = `${nagBanner}<div class="card roi-card" style="margin-bottom:20px;padding:16px;border-radius:12px;background:linear-gradient(135deg,rgba(15,23,42,0.6),rgba(10,20,30,0.6));">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
      <div style="text-align:center;padding:16px;background:rgba(15,23,42,0.5);border-radius:12px;"><div style="font-size:28px;font-weight:bold;color:#22d3ee;">${mockUsage.hours_saved}</div><div style="font-size:12px;color:#64748b;">Hours Saved</div><div style="font-size:11px;color:#10b981;margin-top:4px;">CA$7,100 value</div></div>
      ...
      <div style="text-align:center;padding:16px;background:rgba(15,23,42,0.5);border-radius:12px;"><div style="font-size:28px;font-weight:bold;color:#fbbf24;">${mockUsage.avg_response_time}s</div><div style="font-size:12px;color:#64748b;">Avg Response</div><div style="font-size:11px;color:#10b981;margin-top:4px;">↓8% faster</div></div>
...
/* small helper to show download for a single query (used in UI) */
function downloadQueryTextHandler(q) {
  downloadQueryText(q)
}

/* small helper to expose handleQuery to template (quick action usage) */
function handleQueryShortcut(q) {
  handleQuery(q)
}

/* simple public methods used by template */
const selectIndustryPublic = selectIndustry
const submitQueryPublic = submitQuery

/* expose to template scope */
const exports = {
  selectedIndustry,
  queryInput,
  lastResponse,
  ...
  quickTags,
  getROICard,
  selectIndustry: selectIndustryPublic,
  submitQuery: submitQueryPublic,
...
/* NOTE: Vue <script setup> automatically puts top-level variables into template scope.
   we already declared variables above. */
</script>

<style scoped>
/* preserve small animations and helper classes */
@keyframes spin {
