    ...
    <div id="auth-modal" class="hidden"
        ...
        <div class="glass-card animate-scale-in" style="width:100%;max-width:420px;padding:40px;">
            <div style="text-align:center;margin-bottom:32px;">
                <div class="animate-float" style="width:64px;height:64px;margin:0 auto 16px;"><svg viewBox="0 0 100 100"
                        ...
                        <path d="M15 75 L15 45 L30 55 L50 25 L70 55 L85 45 L85 75 Z" fill="url(#authCrown)"
                            stroke="#38bdf8" stroke-width="2" />
                        <circle cx="50" cy="30" r="6" fill="#fbbf24" />
                        <rect x="20" y="75" width="60" height="8" rx="2" fill="url(#authCrown)" />
                    </svg></div>
                <h2 style="font-size:26px;font-weight:800;">Welcome to <span style="color:#22d3ee;">K.I.N.G.</span></h2>
            </div>
            <div style="display:flex;margin-bottom:24px;background:rgba(51,65,85,0.5);border-radius:12px;padding:4px;">
                <button id="login-tab" onclick="switchAuthTab('login')"
                    style="flex:1;padding:12px;border-radius:10px;border:none;font-size:14px;font-weight:600;cursor:pointer;">Login</button>
                <button id="register-tab" onclick="switchAuthTab('register')"
                    style="flex:1;padding:12px;border-radius:10px;border:none;font-size:14px;font-weight:600;cursor:pointer;">Register</button>
            </div>
<form id="auth-form">
    @csrf
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div id="register-fields" class="hidden">
            ...
            <div>
                ...
                <input
                    ...
                    class="input">
                <small class="error-text" data-error="company"></small>
            </div>
        ...
        <button
            ...
            {{-- <form id="auth-form" onsubmit="handleAuth(event)">
                <div style="display:flex;flex-direction:column;gap:16px;">
                    <div><label class="label">Email</label><input type="email" id="auth-email"
                            placeholder="you@company.com" class="input" required></div>
                    <div><label class="label">Password</label><input type="password" id="auth-password"
                            placeholder="••••••••" class="input" required></div>
                    <div id="register-fields" class="hidden"><label class="label">Company Name</label><input type="text"
                            id="auth-company" placeholder="Your Company" class="input"></div>
                    <button type="submit" class="cta-primary" style="width:100%;padding:16px;margin-top:8px;"
                        id="auth-submit">Sign In</button>
                </div>
            </form> --}}
            <button onclick="closeAuth()"
                style="display:block;margin:24px auto 0;background:none;border:none;color:#64748b;cursor:pointer;font-size:14px;">←
                Back to home</button>
        </div>
    </div>
    ...
    <script>
        ...
        /* ============================
   AUTH FUNCTIONS (NO INLINE)
============================ */

function showAuth(mode) {
    console.log("devloperrr");
...
function switchAuthTab(tab) {
    ...
    if (!lt || !rt || !submitBtn || !registerFields) return;

    if (tab === 'login') {
        lt.style.background = 'linear-gradient(135deg,#0ea5e9,#06b6d4)';
        lt.style.color = '#fff';

        rt.style.background = 'transparent';
        rt.style.color = '#94a3b8';

        submitBtn.textContent = 'Sign In';
        registerFields.classList.add('hidden');
    } else {
        rt.style.background = 'linear-gradient(135deg,#0ea5e9,#06b6d4)';
        rt.style.color = '#fff';

        lt.style.background = 'transparent';
        lt.style.color = '#94a3b8';
...
function handleAuth(e) {
    ...
    submitBtn.textContent =
        APP.authMode === 'login' ? 'Signing in...' : 'Creating account...';
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (typeof renderMobileSidebar === 'function') renderMobileSidebar();
            if (typeof setActiveTab === 'function') setActiveTab('query');
        }, 100);
...
$(document).ready(function () {
    ...
    $('[data-auth-open]').on('click', function () {
        showAuth($(this).data('auth-open'));
    });

    // Close modal
    $('#auth-close').on('click', closeAuth);
...
function handleAuth() {
    ...
    const data = {
        ...
        password_confirmation: $('#auth-password-confirm').val(),
        _token: $('meta[name="csrf-token"]').attr('content')
    };

    if (mode === 'register') {
        data.company = $('#auth-company').val();
    }
    ...
    $.ajax({
        ...
        error: function (xhr) {
            ...
            // ✅ Validation errors
            if (xhr.status === 422) {
                bindErrors(xhr.responseJSON.errors);
                return;
            }
...
/**
 * BIND LARAVEL ERRORS TO FIELDS
 */
function bindErrors(errors) {
    $.each(errors, function (field, messages) {
        const input = $('[name="' + field + '"]');
        const errorBox = $('[data-error="' + field + '"]');
